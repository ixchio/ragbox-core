...
class RAGBox:
    ...
    def __init__(self, document_dir: str | Path, config: Optional[Settings] = None) -> None:
        """
        ...
        self.vector_store = VectorStoreAutoDetector.detect(self.config, self.embedding_provider)
    ...
    def query(self, question: str) -> str:
        ...
        try:
            loop = asyncio.get_running_loop()
            if loop.is_running():
                raise RuntimeError("Cannot call synchronous `query` inside an active event loop. Use `aquery`.")
        except RuntimeError:
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
    ...
    def estimate_cost(self, query: Optional[str] = None) -> str:
        ...
        estimator = CostEstimator(self.llm_client.model_name if hasattr(self.llm_client, "model_name") else "gpt-4o")
        ...
        if query is None:
            ...
            # Embedding cost
            embed_cost = (estimated_tokens / 1_000_000) * 0.13 # large embedding assumption
            ...
            try:
                loop = asyncio.get_running_loop()
                if loop.is_running():
                    raise RuntimeError("Cannot call synchronous `estimate_cost` inside loop.")
            except RuntimeError:
                loop = asyncio.new_event_loop()
                asyncio.set_event_loop(loop)
