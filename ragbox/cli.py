...
def init(
    ...
    """
    console.print(Panel(f"Initializing RAGBox at [cyan]{document_dir}[/cyan]", border_style="green"))
    ...
    try:
        ...
        console.print(Panel(f"[bold yellow]{cost_estimate}[/bold yellow]", title="Cost Estimate"))
        
        # RAGBox __init__ calls _ensure_built which builds synchronously implicitly via asyncio
        with console.status("[bold green]Building Vector Store and Knowledge Graph...", spinner="dots"):
            rg = rg_pre # It's already built by the init above
        console.print("[bold green]✓ Indexing complete.[/bold green] RAGBox is ready for queries.")
        console.print("The self-healing daemon is now active in the background for this session.")
...
def query(
    ...
    try:
        ...
        with console.status("[bold green]Agentic Orchestrator thinking...", spinner="dots"):
