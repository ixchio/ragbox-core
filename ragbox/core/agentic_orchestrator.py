...
class AgenticOrchestrator:
    ...
    async def _expand_query(self, query: str) -> list[str]:
        ...
        """
        try:
            res = await self.llm.agenerate(prompt, system="You are an expert search query generator.")
    ...
    async def execute(self, query_text: str, force_strategy: Optional[RAGStrategy] = None) -> Answer:
    ...
    async def _execute_vector(self, query: str) -> Answer:
        ...
        response = await self.llm.agenerate(prompt, system="You are an expert Q&A engine. Be concise and accurate.")
    ...
    async def _execute_multi_query(self, query: str) -> Answer:
        ...
        response = await self.llm.agenerate(prompt, system="You are an expert Q&A engine analyzing across multiple perspectives.")
    ...
    async def _execute_graph(self, query: str) -> Answer:
        ...
        response = await self.llm.agenerate(prompt, system="You are a graph-aware reasoning agent.")
    ...
    async def _execute_agentic(self, query: str) -> Answer:
        ...
        for step in range(max_steps):
            ...
            action = await self.llm.agenerate(prompt, system="You are a ReAct agent. You can SEARCH or ANSWER.")
            ...
            if "ANSWER" in action:
                ...
                history.append(f"Searched: {search_term}\nFound context length: {len(context)}")
            else:
                break
