...
class OptimizedKnowledgeGraph:
    ...
    def __init__(self, optimize_threshold: int = 100, llm_client: Optional[LLMClient] = None):
        self.graph = nx.Graph()
        self.llm = llm_client
    ...
    def add_document(self, doc_id: str, entities: List[str], relationships: List[Dict[str, Any]]) -> None:
        ...
        for entity in entities:
            if not self.graph.has_node(entity):
                ...
                self.graph.nodes[entity]['weight'] = self.graph.nodes[entity].get('weight', 1.0) + 0.1
    ...
    def _optimize_graph(self) -> None:
        ...
        logger.info(f"Optimizing Knowledge Graph (Nodes: {len(self.graph.nodes)}, Edges: {len(self.graph.edges)})")
    ...
    async def build_from_documents(self, documents: List[Document]) -> None:
        ...
        logger.info(f"Extracting entities and relations for {len(documents)} documents.")
        ...
        for doc in documents:
            ...
            doc_rels = [{"source": r.source_id, "target": r.target_id, "type": r.relation_type} for r in relations]
            self.add_document(doc_id=doc.id, entities=doc_entities, relationships=doc_rels)
    ...
    async def _extract_graph_data(self, documents: List[Document]) -> Tuple[List[Entity], List[Relation]]:
        entities = []
        relations = []
        
        if not getattr(self, "llm", None):
            logger.warning("No LLM client provided to Knowledge Graph. Extracting mock graph.")
            for doc in documents:
                e1 = Entity(id=f"e_{doc.id}_1", name=f"Topic_{doc.id[:4]}", entity_type="CONCEPT", description="Auto-extracted")
                e2 = Entity(id=f"e_{doc.id}_2", name=f"Detail_{doc.id[:4]}", entity_type="DETAIL", description="Auto-extracted")
                entities.extend([e1, e2])
                relations.append(Relation(source_id=e1.id, target_id=e2.id, relation_type="RELATES_TO", context="dummy"))
        ...
        for doc in documents:
            try:
                # Truncate text to avoid exceedingly long LLM calls
                snippet = doc.content[:3000]
                res = await self.llm.agenerate_structured(
                    prompt_t.format(text=snippet), 
                    schema, 
    ...
    async def query(self, query: str, mode: str = "hybrid") -> GraphQueryResult:
        ...
        for i, (comm_idx, node_set) in enumerate(self.communities.items()):
            if i >= 2: break
            summary = self.community_summaries.get(comm_idx) or f"Community {comm_idx} summary."
        ...
        return GraphQueryResult(
            ...
            synthesized_context="\n".join([c.summary for c in communities]) if communities else ""
        )
