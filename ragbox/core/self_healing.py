...
class ProductionFileWatcher(FileSystemEventHandler):
    ...
    def on_modified(self, event):
        ...
        self._pending_queue.append(FileEvent(
            path=file_path,
            event_type="modified",
        ...
        try:
            while self._pending_queue and not self._shutdown:
                ...
                while (
                    ...
                    time.time() - batch_start < self.batch_interval
                ):
...
class SelfHealer:
    ...
    def start_watchdog(self) -> None:
        ...
        self.observer.start()
        logger.info("Watchdog file watcher started with ProductionFileWatcher debouncing.")
    ...
    async def handle_file_event(self, event_type: str, path: Path) -> None:
        ...
        if event_type == "deleted":
            logger.warning(f"File deletion handling not fully implemented for DB delete: {path}")
            return
            ...
            for chunk in chunks:
                if "embedding" in chunk.metadata:
                    vectors_to_index.append({
                        ...
                        "metadata": {"doc_id": chunk.document_id, **{k:v for k,v in chunk.metadata.items() if k != "embedding"}},
