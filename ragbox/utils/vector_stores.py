...
class ChromaStore(VectorStore):
    def __init__(self, persist_dir: str = "./chroma_db", embedding_function: Any = None):
        try:
            import chromadb
    ...
    async def add_documents(self, documents: List[Dict[str, Any]], namespace: Optional[str] = None) -> None:
        if not documents:
            return
    ...
    async def search(
        ...
        for match in res.matches:
            if match.score >= min_score:
                parsed.append(VectorQueryResult(
                    id=match.id,
        ...
        try:
            if ids:
                await asyncio.to_thread(self.index.delete, ids=ids, namespace=namespace)
                 ...
                 await asyncio.to_thread(self.index.delete, filter=filter_dict, namespace=namespace)
            return len(ids) if ids else 1
        except Exception as e:
...
class VectorStoreAutoDetector:
    ...
    def detect(config: Settings, embedding_provider: EmbeddingProvider) -> VectorStore:
        pinecone_key = getattr(config, "pinecone_api_key", os.getenv("PINECONE_API_KEY"))
        if pinecone_key:
            env = getattr(config, "pinecone_env", os.getenv("PINECONE_ENV", "us-east-1"))
            index = getattr(config, "pinecone_index", os.getenv("PINECONE_INDEX", "ragbox"))
            logger.info(f"Using Pinecone as vector store (index: {index}).")
            return PineconeStore(api_key=pinecone_key, environment=env, index_name=index, dimension=embedding_provider.dimension)
            
        logger.info("Using local ChromaDB as vector store.")
        chroma_dir = getattr(config, "chroma_db_dir", os.getenv("CHROMA_DB_DIR", "./chroma_db"))
